...
class ProductController extends Controller
{
    ...
    private function validated(Request $request, ?Product $product = null): array { $data=$request->validate(['slug' => ['nullable','max:255'], 'title_uk' => ['required','max:255'], 'title_es' => ['required','max:255'], 'subtitle_uk' => ['nullable','max:255'], 'subtitle_es' => ['nullable','max:255'], 'description_uk' => ['required'], 'description_es' => ['required'], 'benefits_uk' => ['nullable'], 'benefits_es' => ['nullable'], 'composition_uk' => ['nullable'], 'composition_es' => ['nullable'], 'application_uk' => ['nullable'], 'application_es' => ['nullable'], 'image' => ['nullable','image','max:8192'], 'volume' => ['required','max:50'], 'sort_order' => ['nullable','integer','min:0'], 'featured' => ['nullable'], 'is_active' => ['nullable']]); return array_merge($data,['slug' => Str::slug($request->slug ?: $request->title_es), 'featured' => $request->boolean('featured'), 'is_active' => $request->boolean('is_active')]); }
